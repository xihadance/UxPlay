...
-   Cross-platform screensaver inhibition with the new option `-scrsv <n>` (e.g., to
    ...
    On Linux/*BSD, you can watch output of `dbus-monitor` to verify that inhibition is working. _Might not work on Wayland_.
...
_Note: For Linux/*BSD, to verify this feature is working, you can use `dbus-monitor`
to view events on the D-Bus; depending on the Desktop Environment,
commands like
`gnome-session-inhibit -l`,  ``xfce4-screensaver-command -q``, etc.,
should list UxPlay when it is inhibiting the screensaver._
...
specified). (Cross-platform) option -scrsv <n> to inhibit screensaver while UxPlay
is running (Windows, Linux/*BSD). Add password support (-pw)  using a displayed
